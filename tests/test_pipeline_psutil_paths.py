...
from omoai.api.models import PipelineRequest, OutputFormatParams
...
@pytest.mark.asyncio
async def test_full_pipeline_runs_with_psutil_present_and_unpatched_open(monkeypatch, temp_dir):
    ...
    class _Cfg: pass
    class _Api: pass
    cfg = _Cfg(); cfg.api = _Api(); cfg.api.temp_dir = temp_dir
    ...
    def _asr(audio_path, output_path, config_path=None):
        asr_obj = {"segments": [{"start": 0, "end": 1, "text": "hello"}], "text": "hello"}
    ...
    upload = UploadFile(filename="test.wav", content_type="audio/wav", file_data=b"dummy-bytes")
...
async def test_full_pipeline_runs_with_open_patched(monkeypatch, temp_dir):
    ...
    def _asr(audio_path, output_path, config_path=None):
        asr_obj = {"segments": [{"start": 0, "end": 1, "text": "xin chao"}], "text": "xin chao"}
        Path(output_path).write_text(json.dumps(asr_obj), encoding="utf-8")
    ...
    params = OutputFormatParams(summary="bullets", summary_bullets_max=1, include=["segments"])
