...
def test_asr_script_import():
    ...
    if result.returncode == 0:
        assert (
            "usage:" in result.stdout or "ASR wrapper for ChunkFormer" in result.stdout
        )
        return

    # If it fails, make sure it's not due to ModuleNotFoundError
    assert result.returncode != 1, (
        f"Script failed with ModuleNotFoundError: {result.stderr}"
    )
...
def test_ensure_chunkformer_on_path_function():
    ...
    try:
        ...
        sys.path = [
            p for p in sys.path if str(Path(__file__).parent.parent / "src") not in p
        ]
