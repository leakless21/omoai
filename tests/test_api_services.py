...
class TestPreprocessAudioService:
    ...
    async def test_preprocess_audio_script_failure(self, mock_get_config, mock_run_script, temp_dir, mock_upload_file):
        ...
        mock_run_script.side_effect = subprocess.CalledProcessError(
            returncode=1, cmd="preprocess_script", stderr="Script failed"
        )
        
        request = PreprocessRequest(audio_file=mock_upload_file)
        
        # Execute and verify exception
        with pytest.raises(AudioProcessingException, match="Audio preprocessing failed"):
    ...
    async def test_preprocess_audio_unexpected_error(self, mock_get_config, mock_run_script, temp_dir, mock_upload_file):
        ...
        # Execute and verify exception
        with pytest.raises(AudioProcessingException, match="Unexpected error during preprocessing"):
            await preprocess_audio_service(request)
...
class TestASRService:
    ...
    def test_asr_service_script_failure(self, mock_exists, mock_get_config, mock_run_script, temp_dir):
        ...
        # Execute and verify exception
        with pytest.raises(AudioProcessingException, match="Unexpected error during ASR"):
            asr_service(request)
...
class TestPostprocessService:
    ...
    async def test_postprocess_service_success(self, mock_get_config, mock_run_script, temp_dir):
        ...
        # Mock the script execution and file reading
        with patch('builtins.open', mock_open(read_data=json.dumps(mock_postprocess_result))):
            mock_run_script.return_value = None
            ...
            assert result.summary["abstract"] == "Một đoạn văn bản ngắn bằng tiếng Việt."
