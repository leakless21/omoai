...
class PerformanceMonitor:
    ...
    def stop_monitoring(self) -> dict[str, Any]:
        ...
        if HAS_TORCH and torch.cuda.is_available():
            gpu_memory_values = [
                s.get("gpu_memory_mb", 0) for s in self.samples if "gpu_memory_mb" in s
            ]
            if gpu_memory_values:
                results["peak_gpu_memory_mb"] = max(gpu_memory_values)
                results["avg_gpu_memory_mb"] = sum(gpu_memory_values) / len(
                    gpu_memory_values
                )
...
class LoadTestRunner:
    ...
    def __init__(
        self, operation_func: Callable, operation_name: str = "test_operation"
    ):
    ...
    def run_single_operation(self, *args, **kwargs) -> PerformanceMetrics:
        ...
        try:
            ...
            if (
                initial_gpu_memory is not None
                and HAS_TORCH
        ...
        def run_user_operations(user_id: int) -> list[PerformanceMetrics]:
            ...
            for i in range(operations_per_user):
                try:
                    ...
                    errors.append(f"User {user_id}, Op {i}: {e!s}")
    ...
    def run_stress_test(
        ...
        # Phase 1: Ramp-up
        print(
            f"Starting stress test ramp-up phase ({config.ramp_up_duration_seconds}s)"
        )
        ramp_up_users = list(
            range(
                1,
                config.max_concurrent_users + 1,
                max(1, config.max_concurrent_users // 10),
            )
        )
        ...
        for users in ramp_up_users[:3]:  # Test first few user counts during ramp-up
            ...
            if (
                ...
                and result.peak_memory_mb > config.memory_limit_mb
            ):
                print(
                    f"Memory limit exceeded at {users} users: {result.peak_memory_mb}MB"
                )
                break
        ...
        peak_result = self.run_load_test(
            concurrent_users=min(
                config.max_concurrent_users * 2, 20
            ),  # 2x users but cap at 20
...
class PerformanceTestSuite:
    ...
    def run_baseline_tests(
        ...
        for test_name, test_func in test_functions.items():
            ...
            avg_memory = sum(m.memory_usage_mb for m in metrics_list) / len(
                metrics_list
            )
    ...
    def run_regression_tests(
        ...
        for test_name, test_func in test_functions.items():
            ...
            print(
                f"  Duration ratio: {duration_ratio:.2f}x {'(REGRESSION)' if duration_regression else '(OK)'}"
            )
            print(
                f"  Memory ratio: {memory_ratio:.2f}x {'(REGRESSION)' if memory_regression else '(OK)'}"
            )
            ...
            for test_name, result in results["regression"].items():
                if result["overall_regression"]:
                    ...
                    if result["duration_regression"]:
                        report.append(
                            f"   Duration: {result['duration_ratio']:.2f}x slower"
                        )
