...
class PerformanceBenchmark:
    ...
    expected_gpu_memory_mb: Optional[float]
...
class ReferenceDataManager:
        ...
        if performance_file.exists():
            with open(performance_file, 'r') as f:
                ...
                for name, perf_data in data.items():
                    self.performance_benchmarks[name] = PerformanceBenchmark(**perf_data)
        else:
    ...
    def _save_transcripts(self):
        ...
        data = {name: asdict(transcript) for name, transcript in self.transcripts.items()}
    ...
    def _save_performance_benchmarks(self):
        ...
        data = {name: asdict(benchmark) for name, benchmark in self.performance_benchmarks.items()}
        with open(self.reference_dir / "performance.yaml", 'w') as f:
            yaml.dump(data, f, default_flow_style=False, sort_keys=False)
    
    def _save_quality_benchmarks(self):
        """Save quality benchmarks to file."""
        data = {name: asdict(benchmark) for name, benchmark in self.quality_benchmarks.items()}
        with open(self.reference_dir / "quality.yaml", 'w') as f:
            yaml.dump(data, f, default_flow_style=False, sort_keys=False)
    
    def get_reference_transcript(self, fixture_name: str) -> Optional[ReferenceTranscript]:
        """Get reference transcript for a fixture."""
        return self.transcripts.get(fixture_name)
    
    def get_performance_benchmark(self, fixture_name: str) -> Optional[PerformanceBenchmark]:
        """Get performance benchmark for a fixture."""
        return self.performance_benchmarks.get(fixture_name)
    
    def get_quality_benchmark(self, fixture_name: str) -> Optional[QualityBenchmark]:
        ...
        if "duration_ms" in actual_results:
            ...
            if current_benchmark:
                ...
                self.performance_benchmarks[fixture_name] = PerformanceBenchmark(
                    ...
                    max_processing_time_ms=actual_results["duration_ms"] * performance_margin,
                    max_real_time_factor=rtf * performance_margin,
                    max_memory_usage_mb=actual_results.get("memory_usage_mb", 100.0) * performance_margin,
        ...
        if perf_benchmark and "duration_ms" in actual_results:
            ...
            if "real_time_factor" in actual_results:
                validation_results["performance_rtf"] = (
                    actual_results["real_time_factor"] <= perf_benchmark.max_real_time_factor
                )
        ...
        if quality_benchmark:
            if "confidence_avg" in actual_results:
                validation_results["quality_confidence"] = (
                    actual_results["confidence_avg"] >= quality_benchmark.min_confidence_score
                )
            
            if "word_accuracy" in actual_results:
                validation_results["quality_accuracy"] = (
                    actual_results["word_accuracy"] >= quality_benchmark.min_word_accuracy
                )
