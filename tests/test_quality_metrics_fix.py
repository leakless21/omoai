...
from omoai.api.models import PipelineResponse, QualityMetrics, HumanReadableDiff
...
class TestQualityMetricsFix:
    ...
    def test_quality_metrics_included_when_requested_and_available(self):
        ...
        # Assert
        assert response.quality_metrics is not None, "Quality metrics should not be None when requested and available"
        assert response.diffs is not None, "Diffs should not be None when requested and available"
        assert response.quality_metrics.wer == 0.05, "WER should be 0.05"
        assert response.quality_metrics.cer == 0.03, "CER should be 0.03"
        assert response.diffs.original_text == "bad sentence", "Original text should match"
        assert response.diffs.punctuated_text == "Good sentence.", "Punctuated text should match"
    ...
    def test_quality_metrics_excluded_when_not_requested(self):
        ...
        # Assert
        assert response.quality_metrics is None, "Quality metrics should be None when not requested"
    ...
    def test_quality_metrics_none_when_not_available_in_data(self):
        ...
        # Assert
        assert response.quality_metrics is None, "Quality metrics should be None when not available in data"
    ...
    def test_quality_metrics_partial_data(self):
        ...
        mock_result.segments = [{"start": 0.0, "end": 1.0, "text": "This is a test transcript"}]
        ...
        # Assert
        assert response.quality_metrics is not None, "Quality metrics should not be None"
        assert response.quality_metrics.wer == 0.05, "WER should be 0.05"
        assert response.quality_metrics.cer is None, "CER should be None"
        assert response.quality_metrics.alignment_confidence == 0.95, "Alignment confidence should be 0.95"
    ...
    def test_diffs_partial_data(self):
        ...
        assert response.diffs.alignment_summary == "Punctuation improved", "Alignment summary should match"
