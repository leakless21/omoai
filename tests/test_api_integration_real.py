...
def create_test_wav_file(
    file_path, duration_seconds=3.0, sample_rate=16000, frequency=440
):
...
def create_vietnamese_speech_wav(file_path, duration_seconds=5.0, sample_rate=16000):
    ...
    # segments = []  # Unused variable removed
    ...
    for i, freq in enumerate(base_freqs):
        ...
        tone_patterns = [
            ...
            lambda t: 1
            - 0.3 * t / segment_duration
            + 0.1 * np.sin(2 * np.pi * 5 * t / segment_duration),  # heavy
        ]
...
class TestAPIIntegrationWithRealAudio(unittest.TestCase):
    ...
    def test_asr_endpoint_with_real_audio(self, mock_run_script):
        ...
        with patch("omoai.api.services.run_preprocess_script") as mock_preprocess:
            ...
            mock_preprocess.side_effect = lambda input_path, output_path: Path(
                output_path
            ).touch()
    ...
    @patch("omoai.api.services.run_asr_script")
    @patch("omoai.api.services.run_postprocess_script")
    def test_full_pipeline_endpoint_with_real_audio(
        self, mock_postprocess, mock_asr, mock_preprocess
    ):
    ...
    @patch("omoai.api.services.run_asr_script")
    @patch("omoai.api.services.run_postprocess_script")
    def test_full_pipeline_with_output_parameters(
        self, mock_postprocess, mock_asr, mock_preprocess
    ):
