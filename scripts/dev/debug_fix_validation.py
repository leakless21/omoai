...
def test_fix_validation():
    ...
    print(
        f"  Labels stripped: {not ('Tiêu đề:' in result_1['summary'] or 'Tóm tắt:' in result_1['summary'])}"
    )
    print()
...
- Điểm 3"""
    ...
    print(
        f"  Labels stripped: {not ('Tiêu đề:' in result_3['summary'] or 'Tóm tắt:' in result_3['summary'])}"
    )
    ...
    all_tests_pass = all(
        [
            ...
            not (
                "Tiêu đề:" in result_1["summary"] or "Tóm tắt:" in result_1["summary"]
            ),  # Labels should be stripped
            result_2["title"]
            == "Tích phân đường loại một",  # Proper structure should be preserved
            result_2["summary"]
            == "Bài giảng hướng dẫn cách tính tích phân đường loại một, ứng dụng trong bài toán tính khối lượng dây không đồng chất.",
            bool(result_3["title"]),  # String parsing should extract title
            not (
                "Tiêu đề:" in result_3["summary"] or "Tóm tắt:" in result_3["summary"]
            ),  # String parsing should strip labels
