...
def debug_abstract_pattern():
    ...
    if label_match:
        print(
            f"Label found: '{label_match.group()}' at position {label_match.start()}-{label_match.end()}"
        )
        ...
        if delimiter_match:
            print(
                f"Delimiter found: '{delimiter_match.group()}' at position {delimiter_match.start()}-{delimiter_match.end()}"
            )
