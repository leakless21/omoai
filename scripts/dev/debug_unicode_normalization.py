...
def test_unicode_normalization():
    ...
    for form in forms:
        ...
        for pattern in patterns:
            ...
            print(
                f"  Pattern '{pattern}': {bool(matches)} - {matches[:2] if matches else 'None'}"
            )
    ...
    points_match = re.search(
        r"điểm chính:\s*(.*)", normalized_lower, flags=re.MULTILINE | re.DOTALL
    )
    if points_match:
        points_content = points_match.group(1).strip()
        points = [
            p.strip()
            for p in points_content.split("\n")
