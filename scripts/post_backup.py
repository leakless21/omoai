...
def _force_preserve_with_alignment(original_text: str, llm_text: str, adopt_case: bool = True) -> str:
    ...
    - Tokenize original words and LLM words/punct
    - Align original words to LLM words (ignoring punctuation)
    - Rebuild: insert LLM punctuation around matched positions; use original words;
      if adopt_case=True and words match ignoring case, adopt LLM case
    """
    ...
    # Lowercase for alignment
    a = [w.lower() for w in orig_words]
    b = [w.lower() for w in llm_words]
    sm = SequenceMatcher(None, a, b, autojunk=False)
    # Build mapping from llm word index to list of preceding punctuation token indices
    punct_before: List[List[int]] = [[] for _ in range(len(llm_words) + 1)]
    llw_idx = 0
    for i, tok in enumerate(llm_tokens):
        if _is_word(tok):
            llw_idx += 1
        else:
            punct_before[llw_idx].append(i)
    # Reconstruct tokens
    result_tokens: List[str] = []
    # Track consumed llm word index
    cur_llw = 0
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        if tag in ("equal", "replace", "delete", "insert"):
            # For each original word in this block, attach llm punctuation seen before current llm position
            for i in range(i1, i2):
                # Emit punctuation queued before current llm word index
                for pidx in punct_before[cur_llw]:
                    result_tokens.append(llm_tokens[pidx])
                punct_before[cur_llw] = []
                # Use original word, optionally adopting LLM case
                orig_word = orig_words[i]
                if adopt_case and cur_llw < len(llm_words):
                    llm_word = llm_words[cur_llw]
                    if orig_word.lower() == llm_word.lower():
                        # Words match ignoring case, adopt LLM case
                        orig_word = llm_word
                result_tokens.append(orig_word)
                # Advance llm word index if available
                if cur_llw < len(llm_words):
                    cur_llw += 1
        else:
            continue
    # Flush remaining punctuation
    for pidxs in punct_before[cur_llw:]:
        for pidx in pidxs:
            result_tokens.append(llm_tokens[pidx])
...
def _distribute_punct_to_segments(
    ...
    """Distribute a single punctuated text back to segments by word counts.

    Assumes punctuation/capitalization does not add/remove/reorder words.
    Falls back to leaving text_punct empty on mismatch.
    """
    seg_word_counts = [len(_split_words((s.get("text_raw") or "").strip())) for s in segments]
    total_words_raw = sum(seg_word_counts)
    words_punct = _split_words(punctuated_text)
    if total_words_raw <= 0 or len(words_punct) != total_words_raw:
        # Mismatch: do not attempt risky slicing
        out: List[Dict[str, Any]] = []
        for s in segments:
            out.append({**s, "text_punct": s.get("text_punct", "")})
        return out
...
def _safe_distribute_punct_to_segments(
    ...
    *,
    ...
    """Try strict word-count distribution; if it fails and we have an LLM context,
    re-punctuate per-segment independently. Otherwise, keep strict result as-is.
    """
    strict = _distribute_punct_to_segments(punctuated_text, segments)
    target_nonempty = sum(1 for s in segments if (s.get("text_raw") or "").strip())
    filled = sum(1 for s in strict if (s.get("text_punct") or "").strip())
    if filled >= target_nonempty and target_nonempty > 0:
        return strict
    if llm is not None and system_prompt is not None and max_model_len is not None:
        return _segmentwise_punctuate_segments(
            llm,
            system_prompt,
            int(max_model_len),
            segments,
    ...
    # Fallback to strict (may leave empties) if no LLM context available
    return strict
...
def main() -> None:
    ...
    # Global defaults
    model_id_default = args.model or cfg_get(["llm", "model_id"], "Qwen/Qwen2.5-7B-Instruct-AWQ")
    ...
    for b in batches:
        ...
        # Align LLM output to original words to preserve exact tokens and improve distribution
    ...
    llm_sum = llm_punc if reuse else build_llm(s_model_id, s_quant, s_mml, s_gmu, s_mns, s_mbt)
    try:
