...
def run_postprocess_script(
    ...
    try:
        logger.info(f"Current multiprocessing start method: {multiprocessing.get_start_method()}")
    except Exception:
    ...
    env["CUDA_VISIBLE_DEVICES"] = env.get("CUDA_VISIBLE_DEVICES", "0")  # Ensure consistent GPU visibility
    ...
    logger.info(f"CUDA environment: CUDA_VISIBLE_DEVICES={env.get('CUDA_VISIBLE_DEVICES', 'not set')}")
